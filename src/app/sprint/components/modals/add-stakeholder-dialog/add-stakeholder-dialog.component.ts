...
@Component({
  selector: 'app-add-stakeholder-dialog',
  templateUrl: './add-stakeholder-dialog.component.html',
  styleUrls: ['./add-stakeholder-dialog.component.scss'],

})
export class AddStakeholderDialogComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder,private _snackBar: MatSnackBar, private spinner: NgxSpinnerService, private http: HttpClient, @Inject(MAT_DIALOG_DATA) public data: any, public dialogRef: MatDialogRef<AddStakeholderDialogComponent>) {
  ...
  onSubmit() {
    ...
    // console.log('iddd',this.data.data.description.action.id)
      this.apiData = {
        ...
        isCustom: this.data.action.isCustom ? this.data.action.isCustom : false
      }
    ...
    this.http.post(`${environment.baseurl}/actionstakeholder/add-sel-activity`, this.apiData).subscribe(
      (res: any) => {
        this.spinner.hide();
        console.log('res', res);
        this.dialogRef.close({isCancel: false, added: res.data});
      },
      (err) => {
        console.error(err);
        if(err.status === 400){
